...
@SpellHandler(name = "Enchant Infinity",
    description = "Enchants a bow with Infinity",
    permission = "merlin.cast.enchantment.infinity")
public class EnchantInfinity implements MagicSpell
  {
    public boolean cast (Sorcerer sorcerer, Trigger trigger, Event event)
      {
        ...
        if (isTool(hand))
          {
            int level = 1;
            hand.addEnchantment(Enchantment.ARROW_INFINITE, level);
            return true;
          }
        else
          {
