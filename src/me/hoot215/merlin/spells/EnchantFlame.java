...
@SpellHandler(name = "Enchant Flame",
    description = "Enchants a bow with Flame",
    permission = "merlin.cast.enchantment.flame")
public class EnchantFlame implements MagicSpell
  {
    public boolean cast (Sorcerer sorcerer, Trigger trigger, Event event)
      {
        ...
        if (isTool(hand))
          {
            int level = 1;
            hand.addEnchantment(Enchantment.ARROW_FIRE, level);
            return true;
          }
        else
          {
