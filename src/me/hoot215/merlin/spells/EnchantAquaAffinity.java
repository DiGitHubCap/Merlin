...
@SpellHandler(name = "Enchant Aqua Affinity",
    description = "Enchants a helmet with Aqua Affinity",
    permission = "merlin.cast.enchantment.aquaaffinity")
public class EnchantAquaAffinity implements MagicSpell
  {
    ...
    public boolean cast (Sorcerer sorcerer, Trigger trigger, Event event)
      {
        ...
        if (isHelmet(hand))
          {
            int level = 1;
            hand.addEnchantment(Enchantment.WATER_WORKER, level);
            return true;
          }
        else
          {
