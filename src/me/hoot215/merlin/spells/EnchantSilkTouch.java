...
@SpellHandler(name = "Enchant Silk Touch",
    description = "Enchants a tool with Silk Touch",
    permission = "merlin.cast.enchantment.silktouch")
public class EnchantSilkTouch implements MagicSpell
  {
    ...
    public boolean cast (Sorcerer sorcerer, Trigger trigger, Event event)
      {
        ...
        if (isTool(hand))
          {
            int level = 1;
            hand.addEnchantment(Enchantment.SILK_TOUCH, level);
            return true;
          }
        else
          {
